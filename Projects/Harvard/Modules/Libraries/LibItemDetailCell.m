...
@implementation LibItemDetailCell

@synthesize textLabelNumberOfLines, detailTextLabelNumberOfLines;
@synthesize itemAvailabilityStringAndStatus;
...
{
    CGFloat textWidth = [LibItemDetailCell widthForTextLabel:YES cellStyle:style tableView:tableView accessoryType:accessoryType cellImage:cellImage];
    CGFloat detailTextWidth = [LibItemDetailCell widthForTextLabel:NO cellStyle:style tableView:tableView accessoryType:accessoryType cellImage:cellImage];
	
    if (font == nil) font = [UIFont fontWithName:STANDARD_FONT size:CELL_STANDARD_FONT_SIZE];
    CGFloat textHeight = [LibItemDetailCell heightForLabelWithText:text font:font width:textWidth maxLines:maxTextLines];
	
    CGFloat detailTextHeight = 0.0;
    if (detailText) {
        if (detailFont == nil) detailFont = [UIFont fontWithName:STANDARD_FONT size:CELL_DETAIL_FONT_SIZE];
        detailTextHeight = [LibItemDetailCell heightForLabelWithText:detailText font:detailFont width:detailTextWidth maxLines:maxDetailLines];
    }
	...
	double additionalHeight = 	[@"just need a one liner string"
										sizeWithFont:[UIFont fontWithName:STANDARD_FONT	size:13]
										constrainedToSize:CGSizeMake(300, 70)         
										lineBreakMode:UILineBreakModeWordWrap].height;
	
	additionalHeight = (additionalHeight + 6.0)* [itemAvailabilityDictionary count];
	
    return result + additionalHeight;
}
...
- (void)layoutSubviews {
    ...
    if (textLabelNumberOfLines != 1) {
        ...
        frame.size.width = [LibItemDetailCell widthForTextLabel:YES cellStyle:_style tableView:tableView accessoryType:accessoryType cellImage:YES];
		frame.size.width = frame.size.width; //frame.size.width - 30;
        frame.size.height = [LibItemDetailCell heightForLabelWithText:self.textLabel.text
    ...
    if (self.detailTextLabel.text && detailTextLabelNumberOfLines != 1) {
        ...
        frame.size.width = [LibItemDetailCell widthForTextLabel:NO cellStyle:_style tableView:tableView accessoryType:accessoryType cellImage:YES];
        frame.size.height = [LibItemDetailCell heightForLabelWithText:self.detailTextLabel.text
	...
	frame = self.detailTextLabel.frame;
	frame.origin.y = self.textLabel.frame.origin.y + self.textLabel.frame.size.height;
	...
	for (int index = 0; index < [itemAvailabilityStringAndStatus count]; index ++) {
		...
		additionalHeight += 5.0;
		
		NSString * itemStatus =  [itemAvailabilityStringAndStatus objectForKey:[[itemAvailabilityStringAndStatus allKeys] objectAtIndex:index]];
...
- (void)dealloc {
    self.itemAvailabilityStringAndStatus = nil;
