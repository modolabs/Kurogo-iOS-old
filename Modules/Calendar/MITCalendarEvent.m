...
- (void)updateWithDict:(NSDictionary *)dict
{
	...
	if ([dict objectForKey:@"infophone"] != [NSNull null]) {
		NSString *phoneNumber = [dict objectForKey:@"infophone"];
	...
	if (catArray = [dict objectForKey:@"categories"]) {
		for (NSDictionary *catDict in catArray) {
			...
			NSInteger catID = [name isEqualToString:@"Exhibits"] ? kCalendarExhibitCategoryID : [[catDict objectForKey:@"catid"] intValue];
